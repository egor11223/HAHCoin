...
KeePass integration use KeePassHttp (https://github.com/pfn/keepasshttp/) to facilitate comhahications between the client and KeePass. KeePassHttp is a plugin for KeePass 2.x and provides a secure means of exposing KeePass entries via HTTP for clients to consume.

The implementation is dependent on the following:
 - crypter.h for AES encryption helper functions.
 - rpcprotocol.h for handling RPC comhahications. Could only be used partially however due some static values in the code.
 ...
 - JSON Spirit for reading / writing RPC comhahications

### What's new

The following new options are available for hahd and hah-qt:
 ...
 - _-keepasskey=_ KeePassHttp key for AES encrypted comhahication with KeePass
...
The following rpc commands are available:
 ...
 - _keepass genkey_: generates a base64 encoded 256 bit AES key that can be used for the comhahication with KeePassHttp. Only necessary for manual configuration. Use init for automatic configuration.
 - _keepass init_: sets up the association between hahd and keepass by generating an AES key and sending an association message to KeePassHttp. This will trigger KeePass to ask for an Id for the association. Returns the association and the base64 encoded string for the AES key.
...
Sample initialization flow from _hah-qt_ console (this needs to be done only once to set up the association):

 - Have KeePass running with an open database
 - Start _hah-qt_
 - Open console
 - Type "_keepass init_" in hah-qt console
 - Keepass pops up and asks for an association id, fill that in, for example, "_mydrkwallet_"
 - You should get a response like this "_Association successful. Id: mydrkwallethah - Key: AgQkcs6cI7v9tlSYKjG/+s8wJrGALHl3jLosJpPLzUE=_"
 - Edit _hah.conf_ and fill in these values
```
keepass=1
...
Extended guide with screenshots is also available: https://hahtalk.org/threads/keepass-integration.3620/
